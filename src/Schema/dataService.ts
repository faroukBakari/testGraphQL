...
export default class dataService {
    ...
    private static rowAsObj(tab: string[], id: number) : Object {
        if (id in tab) {
            return dataService.parseWithId(tab[id], id);
        } else {
            throw new Error(`record id [${id}] not found`);
        }
    };
    static getTable(tabName: string): Object[] | undefined {
        if (!dataService.tables.has(tabName)) throw new Error(`table ${tabName} not found`);
        return dataService.tables.get(tabName)?.map((row, id) => dataService.parseWithId(row, id));
    };
    static getTableRow(tabName: string, id: number): Object | undefined {
        const tab = dataService.tables.get(tabName);
        if (tab) {
            return dataService.rowAsObj(tab, id);
        } else {
    ...
    static filterTable(tabName: string, filter: (obj: Object) => boolean): Object[] | undefined {
        if (!dataService.tables.has(tabName)) throw new Error(`table ${tabName} not found`);
        return dataService.tables.get(tabName)?.map((row, id) => dataService.parseWithId(row, id)).filter(filter);
    };
    static insertTableRow(tabName: string, obj: Object): Object {
        const tab = dataService.tables.get(tabName);
        if (tab) {
            tab.push(JSON.stringify(obj));
            return dataService.rowAsObj(tab, (tab.length - 1));
        } else {
            throw new Error(`table ${tabName} not found`);
        }
    };
